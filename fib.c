...
unsigned long long memoized_recursive_fib(int position) {
    ...
    if (position == 0) {
        ...
        fib_cache[position] = memoized_recursive_fib(position - 1) + memoized_recursive_fib(position - 2);
    }
    return fib_cache[position];
}
...
int main(int argc, char *argv[]) {
    ...
    if (method == 'r') {
        fib_cache = (unsigned long long *)malloc((position + 1) * sizeof(unsigned long long));
